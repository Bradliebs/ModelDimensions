...
# GPT-2 end-of-text token. Used to right-pad neighbors so they look the way
# they did at training time (cell_tokens.npy was built with EOT padding).
EOT_TOKEN = 50256
...
def retrieve_neighbors_for_chunks(
    ...
    for ci, chunk_text in enumerate(chunks_text):
        ...
        for ni, hit in enumerate(hits[:n_neighbors]):
            ...
            # Right-pad with EOT to match training distribution
...
def main():
    ...
    parser.add_argument("--no-retrieval", action="store_true",
                        help="Skip the with-retrieval pass (and the bank check)")
    parser.add_argument("--interactive", action="store_true",
                        help="REPL: read prompts from stdin until EOF or 'quit'")
    args = parser.parse_args()

    torch.manual_seed(SEED)
    np.random.seed(SEED)

    # Verify bank service is up (only needed if we'll actually do retrieval)
    if not args.no_retrieval:
        try:
            _client.post("/query", json={"text": "ping", "top_k": 1}).raise_for_status()
            try:
                info = _client.get("/info").json()
                print(f"Bank: {info['n_cells']:,} cells, dim={info['dim']}")
            except Exception:
                print(f"Bank: reachable at {SERVICE_URL} (/info not available)")
    ...
    if args.interactive:
        print("\nInteractive mode. Type a prompt and press Enter. Empty line or 'quit' to exit.")
        while True:
            try:
                prompt = input("\n> ").strip()
