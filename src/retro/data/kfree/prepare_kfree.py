...
SOURCES:
  ...
  2. Wikipedia EN (100k article subset) — linguistic variety
  3. Existing code tokens (data/code/train_code.bin) — cross-domain
  4. C4 (allenai/c4 EN, streamed) — general web text for language modeling mass

TARGET: ~300M tokens for 404M param model training.
...
VAL_FRACTION = 0.05
SEED = 42
WIKI_ARTICLES = 100_000  # scaled up for larger model
...
def collect_c4(target_tokens: int = 150_000_000) -> list[np.ndarray]:
    """Stream C4 EN for general web text up to target_tokens."""
    print(f"\n[4/4] C4 EN — streaming ~{target_tokens/1e6:.0f}M tokens")
    ...
    for i, row in enumerate(ds):
        ...
        total_tokens += len(arr)
        if (i + 1) % 50_000 == 0:
            print(f"  {i+1:,} docs, {total_tokens:,} tokens so far")

    print(f"  documents: {len(all_token_arrays)}")
    print(f"  tokens: {total_tokens:,}")
    return all_token_arrays
...
def main():
    ...
    # Figure out how many C4 tokens we need to reach ~300M total
    have = sum(len(d) for d in oasst_docs + wiki_docs + code_docs)
    c4_target = max(0, 300_000_000 - have)
    c4_docs = collect_c4(target_tokens=c4_target) if c4_target > 0 else []

    all_docs = oasst_docs + wiki_docs + code_docs + c4_docs
    print(f"\n{'=' * 60}")
    ...
    meta = {
        ...
        "sources": {
            ...
            "c4": {
                "docs": len(c4_docs),
                "tokens": sum(len(d) for d in c4_docs),
            },
        },
    }
