...
> **Scope of this document.** This report covers the **frozen v1.0 research
> baseline** — the verifier-led concept-cell memory captured at tag
> `v1.0-research-baseline`. It is *not* a status report on current work. The
> current effort builds a separate V1 grounded-answer pipeline
> (StreamingBank → silence gate → Phi-3-mini-4k-instruct → token-coverage
> verifier) on top of the same 5.7M-cell bank. For its status, plan and
> deviations see [PLAN.md](PLAN.md). The two artefacts share the bank and
> the encoder; they do not share the generator or the verifier.
